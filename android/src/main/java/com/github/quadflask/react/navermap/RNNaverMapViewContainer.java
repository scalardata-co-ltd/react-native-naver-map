...
public class RNNaverMapViewContainer extends FrameLayout implements RNNaverMapViewProps {
  ...
  public boolean dispatchTouchEvent(MotionEvent ev) {
    if (mapView != null && mapView.getMap().getUiSettings() != null && mapView.getMap().getUiSettings().isScrollGesturesEnabled()) {
      switch (ev.getAction()) {
  ...
  @Override
  public LatLng getPointLatLng(float x, float y) {
    Log.d("container", "getPointLatLng: " + x + ", " + y);
    if (mapView != null)
      return mapView.getPointLatLng(x, y);
    return null;
  }
